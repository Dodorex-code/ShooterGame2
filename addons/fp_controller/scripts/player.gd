...
# footsteps
@onready var dynamic_footsteps: Node3D = $DynamicFootsteps

# dynamic music
@onready var audio_stream_player: AudioStreamPlayer = $AudioStreamPlayer
@onready var audio_stream_player_2d_rain: AudioStreamPlayer2D = $AudioStreamPlayer2D_Rain

# UI
@onready var death_screen: Control = $DeathScreen
@onready var damage_screen: Control = $damage_screen

# light
@onready var light: OmniLight3D = $Placeholder_Collider/Placeholder_BodyMesh/OmniLight3D

# first person arms
@onready var first_person_arms: Node3D = $CameraPivot/SmoothCamera/First_Person_Arms
...
var use_magic:bool = false
var is_attacking_1:bool = false
...
func check_controls() -> void:
	...
	if !InputMap.has_action(INTERACT):
		...
		_add_input_map_event(INTERACT, KEY_E)
...
func respawn() -> void:
	position = spawnpoint
	health = max_health
	damage_screen.update_damage_screen()
...
func deal_damage(damage:float, origin:Node3D, damage_type:int = 0) -> void:
	...
	if health <= 0:
		match damage_type:
			1:
				death_screen.death_msg("You were struck by lightning")
			2:
				death_screen.death_msg("You slipped and fell on a banana peel")
			_:
				death_screen.death_msg("You have been killed by "+str(origin.name))
		can_move = false
		can_jump = false
		can_crouch = false
...
func enter_combat() -> void:
	var sync : AudioStreamSynchronized = audio_stream_player.stream
	sync.set_sync_stream_volume(0, -60)
	sync.set_sync_stream_volume(1, 10)
	audio_stream_player_2d_rain.volume_db = -13.0
...
func exit_combat() -> void:
	var sync : AudioStreamSynchronized = audio_stream_player.stream
	sync.set_sync_stream_volume(0, -60)
	sync.set_sync_stream_volume(1, -60)
	audio_stream_player_2d_rain.volume_db = -3.0
